...
const Header = ({showSidebar}: Props) => {
  ...
  return (
    ...
    <div
      className={`flex flex-col p-3 lg:p-8 w-full fixed top-0 z-10 transition-transform duration-300 max-w-[2800px] sm:pr-[15%] md:pr-[37%] lg:pr-[25%] xl:pr-[20%] 2xl:pr-[17%]  ${
        isVisible ? "translate-y-0" : "-translate-y-full"
      }`}
    >
      <div className="bg-transparent flex flex-row items-center justify-between w-full">
        <img src={images.truLogo} className={`${showSidebar && 'hidden'} w-14 sm:w-16 lg:h-17 lg:w-20 cursor-pointer`} />
