...
const Signup = (props: Props) => {
  ...
  const handleConfirm = async () => {
    try {
      ...
      setLoader(true);
      const response = (await validateEmailAvailability(body)) as {
        data: any;
        success: boolean;
        message: string;
      };
  ...
  const handleConfirmBarber = async () => {
    if (role !== null) {
      if (name && email && password && phone && check) {
        if (password.length < 8) {
          return Toast("error", "Password must contain atleast 8 characters");
        }
        try {
          ...
          if (response.data.success) {
            ...
            navigate("/create-barber-profile", { state: { userData } });
          } else {
            setLoader(false);
            Toast("error", response?.data?.message);
          }
  ...
  return (
    <div className="w-full lg:h-screen flex flex-col lg:flex-row items-start justify-between max-h-screen">
      ...
      <div className="relative lg:h-full bg-white h-[40vh] md:h-[60vh] w-full lg:w-[85%] xl:w-[70%] flex flex-col items-start justify-between pl-4 py-4 lg:pl-10 lg:py-8 bg-cover md:bg-center lg:bg-contain xl:bg-cover bg-no-repeat">
        <img
          src={images.signinBG}
          className="w-full h-full absolute top-0 left-0 p-2 rounded-3xl"
        />
        ...
        <div className="z-10 ">
          ...
          <div className="text-hoverGray mt-4 text-xs md:text-base z-10 w-[95%]">
      ...
      <div className="h-full lg:w-1/2 w-full flex flex-col items-center justify-center lg:justify-between py-8 md:py-14 lg:mt-0 ml-0 lg:ml-4 xl-ml-10 overflow-scroll hide-scrollbar">
        <div className="w-full px-4 w-[100%] sm:w-[80%] md:w-[60%] lg:w-[90%]">
          <div className="flex">
            <div className="bg-inputGray p-2 rounded-lg flex flex-row items-center justify-center mx-auto w-auto mb-4">
              <div
                className={`w-28  text-center py-1 rounded-lg cursor-pointer ${
              ...
              <div
                className={`w-28  text-center py-1 rounded-lg cursor-pointer ${
                  role === "barber" && "bg-black text-white"
                }`}
          ...
          <div className="text-md md:text-xl lg:text-4xl xl:text-4xl mt-2 font-bold w-full">
            Create Your TRU Barber Account
          </div>
          <div className="text-sm font-normal mt-2">
            Sign up today to start your grooming journey. Book appointments,
            manage your profile, and stay updated with the latest trends
          </div>
          <div className="mt-4 lg:mt-8">
            ...
            <div className="bg-inputGray flex flel-row items-center justify-start px-4 rounded-lg mt-2">
              ...
              />
              <img
                src={showPass ? images.eyeOff : images.eye}
          ...
          <Button
            title="Sign Up"
            onClick={() =>
              role === "user" ? handleConfirm() : handleConfirmBarber()
            }
            mt={"50px"}
            loader={loader}
            light={false}
        ...
        <div className="w-full px-4 w-[100%] sm:w-[80%] md:w-[60%] lg:w-[90%] mt-16 md-mt-0">
          <div className="text-center font-semibold text-hoverGray ">
