...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    addMessageInChatRoom(state, action: PayloadAction<any>) {
      ...
      if (state.user) {
        const chatIndex = state.user.chat.findIndex(
          (chat: any) => chat._id === chatRoomId
        );
        if (chatIndex !== -1) {
    ...
    addNewChatInRedux: (state, action: PayloadAction<any>) => {
      ...
      if (state.user) {
        if (
          newChat.user._id === state.user._id ||
          newChat.barber._id === state.user._id
        ) {
    ...
    addAndUpdateNewChatInRedux: (state, action: PayloadAction<any>) => {
      ...
      if (state.user) {
        if (
          ...
          const existingChatIndex = state.user.chat.findIndex(
            (chat: any) => chat._id === newChat._id
          );
    ...
    addNewNotificationRedux: (state, action: PayloadAction<any>) => {
      ...
      if (state.user) {
        if (
          newNotification.user === state.user._id ||
          newNotification.barber === state.user._id
        ) {
    ...
    setNotificationSeenTrueRedux: (state) => {
      if (state.user) {
        if (state.user.role === "user") {
          const updatedNotifications = state.user.notification.map(
            (notif: any) => ({
              ...notif,
              userSeen: true,
            })
    ...
    acceptAppointment: (state, action: PayloadAction<any>) => {
      const { status, acceptAppointment, rejectedAppointment, rejectedStatus } =
        action.payload;
      if (state.user) {
        state.user.appoinment = state.user.appoinment.map(
          (appointment: any) => {
    ...
    //   if (state.user && state.user.services) {
    //     state.user.services = state.user.services.map((service:any) =>
    ...
    updateServiceRedux: (state, action) => {
      ...
      if (state.user && state.user.services) {
        return {
          ...
          user: {
            ...state.user,
            services: state.user.services.map((service: any) =>
              service._id === updatedService._id
                ? {
                    ...service,
                    ...updatedService,
                    options: updatedService.options
                      ? JSON.parse(JSON.stringify(updatedService.options))
                      : service.options,
                  }
                : service
            ),
    ...
    deleteServiceRedux: (state, action) => {
      ...
      if (state.user && state.user.services) {
        state.user.services = state.user.services.filter(
          (service:any) => service._id !== deletedService
        );
      }
    },
  },
